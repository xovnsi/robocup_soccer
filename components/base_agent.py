...
class Agent:
    ...
    def init_connection(self):
        if self.role == Role.GOALIE:
            init_msg = f"(init {self.team.value} (version 15) (goalie))"
        else:
    ...
    def parse_see(self, msg):
        ...
        if ball_match:
            ball_angle = float(ball_match.group(1)) 
            ball_distance = float(ball_match.group(2))  
            return ball_angle, ball_distance  
    ...
    def run(self):
        ...
        while True:
            if self.role == Role.STRIKER:
                ...
                self.sock.sendto("(move 50)".encode(), self.server_addr)
                time.sleep(0.1)
